...
<div class="flex justify-between items-end mb-8">
    <div>
        ...
        <h2 class="text-4xl font-extrabold">Barang</h2>
    </div>

    <a href="{{ route('items.create') }}" class="primary-gradient text-white px-6 py-3 rounded-2xl font-bold">
        + Tambah Item
    </a>
</div>
    ...
    <table class="w-full text-left">
        <thead class="table-head">
            <tr>
                ...
                <th class="px-8 py-5 text-xs uppercase text-slate-400">Kategori</th>
                <th class="px-8 py-5 text-xs uppercase text-slate-400">Satuan</th>
        ...
        <tbody>
            ...
            <tr class="table-row">
                <td class="px-8 py-5 font-mono text-primary font-bold">{{ $item->code }}</td>
                <td class="px-8 py-5 font-bold">{{ $item->name }}</td>
                <td class="px-8 py-5">{{ $item->itemType->name ?? '-' }}</td>
                <td class="px-8 py-5">{{ $item->unit }}</td>
                <td class="px-8 py-5 text-right">
                    <a href="{{ route('items.edit', $item) }}" class="text-primary font-bold">Edit</a>
