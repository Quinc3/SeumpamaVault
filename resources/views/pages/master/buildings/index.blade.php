...
<div class="flex justify-between items-end mb-8">
    <div>
        ...
        <h2 class="text-4xl font-extrabold">Gedung</h2>
    </div>

    <a href="{{ route('buildings.create') }}" class="primary-gradient text-white px-6 py-3 rounded-2xl font-bold">
        + Tambah Gedung
    </a>
</div>
    ...
    <table class="w-full text-left">
        <thead class="table-head">
            <tr>
                ...
                <th class="px-8 py-5 text-xs uppercase text-slate-400">Nama Gedung</th>
        ...
        <tbody>
            ...
            <tr class="table-row">
                <td class="px-8 py-5 font-mono text-primary font-bold">{{ $building->code ?? '-' }}</td>
                <td class="px-8 py-5 font-bold">{{ $building->name }}</td>
                <td class="px-8 py-5 text-slate-500">{{ $building->description ?? '-' }}</td>
                <td class="px-8 py-5 text-right">
                    <a href="{{ route('buildings.edit', $building) }}" class="text-primary font-bold">Edit</a>
            ...
            @empty
            <tr>
                <td colspan="4" class="px-8 py-10 text-center text-slate-400">Belum ada data.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
